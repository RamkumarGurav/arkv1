...
class Expense_Module extends Main {
	...
	function __construct()
	{
		...
		$this->load->model('administrator/master/Expense_Model');
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Expense_Module/list' , $this->data);
		parent::get_footer();
	}
	...
	function expense_list()
	{
		...
		$data_count = $this->Expense_Model->get_expense($search);
		...
		$this->data['expense_data'] = $this->Expense_Model->get_expense($search);
		
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Expense_Module/expense_list' , $this->data);
		parent::get_footer();
	}

	function expense_list_export()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 9;
		$this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id"=>$this->data['page_module_id']));
		...
		$this->load->view('admin/master/Expense_Module/expense_list_export' , $this->data);
	}

	function expense_view($expense_m_id="")
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 9;
		$this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id"=>$this->data['page_module_id']));
		//print_r($this->data['user_access']);
		if(empty($expense_m_id))
		{
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['expense_data'] = $this->Expense_Model->get_expense(array("expense_m_id"=>$expense_m_id));
		if(empty($expense_m_id))
		{
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Something Went Wrong. Please Try Again. </div>';
			$this->session->set_flashdata('alert_message', $alert_message);
		...
		$this->data['expense_data'] = $this->data['expense_data'][0];
		
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Expense_Module/expense_view' , $this->data);
		parent::get_footer();
	}

	function expense_edit($expense_m_id="")
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 9;
		...
		if(!empty($expense_m_id)){
			$this->data['expense_data'] = $this->Expense_Model->get_expense(array("expense_m_id"=>$expense_m_id));
			if(empty($this->data['expense_data']))
			{
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Expense_Module/expense_edit' , $this->data);
		parent::get_footer();
	}

	function userExpenseDoEdit()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 9;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id"=>$this->data['page_module_id']));
		...
		$expense_m_id = $_POST['expense_m_id'];
		//print_r($_POST);
		if(empty($this->data['user_access']))
		{
			REDIRECT(MAINSITE_Admin."wam/access-denied");
		...
		$status = $_POST['status'];
		$is_exist = $this->Common_Model->getData(array('select'=>'*' , 'from'=>'expense_master' , 'where'=>"expense_name = '$expense_name' and expense_m_id != $expense_m_id"));
	//	echo $this->db->last_query();
	//	print_r($is_exist);
		if(!empty($is_exist))
		{
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Expense already exist in database.</div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			//echo $this->session->flashdata('alert_message' );
			//echo "anubhav";
			REDIRECT(MAINSITE_Admin.$user_access->class_name."/expense-edit/".$expense_m_id);
			exit;
		}

		$enter_data['expense_name'] = $expense_name;
		...
		if(!empty($expense_m_id))
		{
			...
			$insertStatus = $this->Common_Model->update_operation(array('table'=>'expense_master', 'data'=>$enter_data, 'condition'=>"expense_m_id = $expense_m_id"));
		...
		else
		{
			...
			$insertStatus = $this->Common_Model->add_operation(array('table'=>'expense_master' , 'data'=>$enter_data));
			if(!empty($insertStatus))
			{
		...
		{
			REDIRECT(MAINSITE_Admin.$user_access->class_name."/expense-edit");
		}

		REDIRECT(MAINSITE_Admin.$user_access->class_name."/".$user_access->function_name);
	}
	
	function userExpense_doUpdateStatus()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 9;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id"=>$this->data['page_module_id']));
		...
		if($user_access->update_module==1)
		{
			...
			if(!empty($id_arr))
			{
				...
				$response = $this->Common_Model->update_operation(array('table'=>"expense_master" , 'data'=>$update_data , 'condition'=>"expense_m_id in ($ids)" ));
				if($response){
