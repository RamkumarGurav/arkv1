...
class Bank_Module extends Main {
	...
	function __construct()
	{
		...
		$this->load->model('administrator/master/Bank_Model');
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Bank_Module/list' , $this->data);
		parent::get_footer();
	}
	...
	function bank_list()
	{
		...
		$data_count = $this->Bank_Model->get_bank($search);
		...
		$this->data['bank_data'] = $this->Bank_Model->get_bank($search);
		
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Bank_Module/bank_list' , $this->data);
		parent::get_footer();
	}
	...
	function bank_list_export()
	{
		...
		$this->load->view('admin/master/Bank_Module/bank_list_export' , $this->data);
	}

	function bank_view($bank_id="")
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 8;
		$this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id"=>$this->data['page_module_id']));
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['bank_data'] = $this->Bank_Model->get_bank(array("bank_id"=>$bank_id));
		if(empty($bank_id))
		{
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Something Went Wrong. Please Try Again. </div>';
			$this->session->set_flashdata('alert_message', $alert_message);
		...
		$this->data['bank_data'] = $this->data['bank_data'][0];
		
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Bank_Module/bank_view' , $this->data);
		parent::get_footer();
	}
	...
	function bank_edit($bank_id="")
	{
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/Bank_Module/bank_edit' , $this->data);
		parent::get_footer();
	}

	function userBankDoEdit()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 8;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id"=>$this->data['page_module_id']));
		
		if(empty($_POST['bank_name']) || empty($_POST['account_holder_name']) || empty($_POST['bank_account_number']) || empty($_POST['bank_ifsc_code']) )
		{
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Something Went Wrong. Please Try Again. </div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			REDIRECT(MAINSITE_Admin.$user_access->class_name."/".$user_access->function_name);
		...
		$status = $_POST['status'];
		$is_exist = $this->Common_Model->getData(array('select'=>'*' , 'from'=>'bank' , 'where'=>"bank_name = '$bank_name' and bank_id != $bank_id"));
	//	echo $this->db->last_query();
	//	print_r($is_exist);
		if(!empty($is_exist))
		{
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Bank already exist in database.</div>';
			$this->session->set_flashdata('alert_message', $alert_message);
			//echo $this->session->flashdata('alert_message' );
			//echo "anubhav";
			REDIRECT(MAINSITE_Admin.$user_access->class_name."/bank-edit/".$bank_id);
			exit;
		}

		$enter_data['bank_name'] = $bank_name;
		$enter_data['account_holder_name'] = $account_holder_name;
		$enter_data['bank_account_number'] = $bank_account_number;
		$enter_data['bank_address'] = $bank_address;
		$enter_data['bank_ifsc_code'] = $bank_ifsc_code;
		...
		if(!empty($bank_id))
		{
			...
			$insertStatus = $this->Common_Model->update_operation(array('table'=>'bank', 'data'=>$enter_data, 'condition'=>"bank_id = $bank_id"));
		...
		else
		{
			...
			$insertStatus = $this->Common_Model->add_operation(array('table'=>'bank' , 'data'=>$enter_data));
			if(!empty($insertStatus))
			{
		...
		{
			REDIRECT(MAINSITE_Admin.$user_access->class_name."/bank-edit");
		}

		REDIRECT(MAINSITE_Admin.$user_access->class_name."/".$user_access->function_name);
	}
	
	function userBank_doUpdateStatus()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 8;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id"=>$this->data['page_module_id']));
		...
		if($user_access->update_module==1)
		{
			...
			if(!empty($id_arr))
			{
				...
				$response = $this->Common_Model->update_operation(array('table'=>"bank" , 'data'=>$update_data , 'condition'=>"bank_id in ($ids)" ));
				if($response){
