...
class T_And_C_Quotation_Module extends Main {
	...
	function __construct() {
		...
		$this->load->model('administrator/master/T_And_C_Quotation_Model');
	...
	function index()
	{
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/T_And_C_Quotation_Module/list' , $this->data);
		parent::get_footer();
	}

	function t_and_c_quotation_list()
	{
		...
		$data_count = $this->T_And_C_Quotation_Model->get_t_and_c_quotation_master($search);
		...
		$this->data['t_and_c_quotation_master_data'] = $this->T_And_C_Quotation_Model->get_t_and_c_quotation_master($search);
		
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/T_And_C_Quotation_Module/t_and_c_quotation_list' , $this->data);
		parent::get_footer();
	}

	function t_and_c_quotation_list_export()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 6;
		$this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id"=>$this->data['page_module_id']));
		...
		$this->load->view('admin/master/T_And_C_Quotation_Module/t_and_c_quotation_list_export' , $this->data);
	}

	function t_and_c_quotation_view($t_and_c_quotation_id="")
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 6;
		$this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id"=>$this->data['page_module_id']));
		//print_r($this->data['user_access']);
		if(empty($t_and_c_quotation_id))
		{
		...
		$this->data['page_is_master'] = $this->data['user_access']->is_master;
		$this->data['page_parent_module_id'] = $this->data['user_access']->parent_module_id;
		$this->data['t_and_c_quotation_master_data'] = $this->T_And_C_Quotation_Model->get_t_and_c_quotation_master(array("t_and_c_quotation_id"=>$t_and_c_quotation_id));
		if(empty($t_and_c_quotation_id))
		{
			$alert_message = '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><i class="icon fas fa-ban"></i> Something Went Wrong. Please Try Again. anubhav</div>';
			$this->session->set_flashdata('alert_message', $alert_message);
		...
		$this->data['t_and_c_quotation_master_data'] = $this->data['t_and_c_quotation_master_data'][0];
		
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/T_And_C_Quotation_Module/t_and_c_quotation_view' , $this->data);
		parent::get_footer();
	}

	function t_and_c_quotation_edit($t_and_c_quotation_id="")
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 6;
		...
		if(!empty($t_and_c_quotation_id)){
			$this->data['t_and_c_quotation_master_data'] = $this->T_And_C_Quotation_Model->get_t_and_c_quotation_master(array("t_and_c_quotation_id"=>$t_and_c_quotation_id));
			if(empty($this->data['t_and_c_quotation_master_data']))
			{
		...
		parent::get_header();
		parent::get_left_nav();
		$this->load->view('admin/master/T_And_C_Quotation_Module/t_and_c_quotation_edit' , $this->data);
		parent::get_footer();
	}

	function userTAndQuotationDoEdit()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 6;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id"=>$this->data['page_module_id']));
		
		if(empty($_POST['t_and_c_quotation_name']) && empty($_POST['term_and_condition']))
		{
		...
		$t_and_c_quotation_id = $_POST['t_and_c_quotation_id'];
		//print_r($_POST);
		if(empty($this->data['user_access']))
		{
			REDIRECT(MAINSITE_Admin."wam/access-denied");
		...
		$t_and_c_quotation_name = trim($_POST['t_and_c_quotation_name']);
		$term_and_condition = $_POST['term_and_condition'];
		
		$status = $_POST['status'];
		$is_exist = $this->Common_Model->getData(array('select'=>'*' , 'from'=>'t_and_c_quotation_master' , 'where'=>"t_and_c_quotation_name = '$t_and_c_quotation_name' and t_and_c_quotation_id != $t_and_c_quotation_id"));
	...
	//	print_r($is_exist);
		if(!empty($is_exist))
		{
			...
			//echo "anubhav";
			REDIRECT(MAINSITE_Admin.$user_access->class_name."/t-and-c-quotation-edit/".$t_and_c_quotation_id);
			exit;
		}

		$enter_data['t_and_c_quotation_name'] = $t_and_c_quotation_name;
		...
		if(!empty($t_and_c_quotation_id))
		{
			...
			$insertStatus = $this->Common_Model->update_operation(array('table'=>'t_and_c_quotation_master', 'data'=>$enter_data, 'condition'=>"t_and_c_quotation_id = $t_and_c_quotation_id"));
		...
		else
		{
			...
			$insertStatus = $this->Common_Model->add_operation(array('table'=>'t_and_c_quotation_master' , 'data'=>$enter_data));
			if(!empty($insertStatus))
			{
		...
		{
			REDIRECT(MAINSITE_Admin.$user_access->class_name."/t-and-c-quotation-edit");
		}

		REDIRECT(MAINSITE_Admin.$user_access->class_name."/".$user_access->function_name);
	}
	
	function userTAndQuotation_doUpdateStatus()
	{
		$this->data['page_type'] = "list";
		$this->data['page_module_id'] = 6;
		$user_access = $this->data['user_access'] = $this->data['User_auth_obj']->check_user_access(array("module_id"=>$this->data['page_module_id']));
		...
		if($user_access->update_module==1)
		{
			...
			if(!empty($id_arr))
			{
				...
				$response = $this->Common_Model->update_operation(array('table'=>"t_and_c_quotation_master" , 'data'=>$update_data , 'condition'=>"t_and_c_quotation_id in ($ids)" ));
				if($response){
