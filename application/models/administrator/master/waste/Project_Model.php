...
class Project_Model extends CI_Model
{
	...
	function get_project($params = array())
	{
		...
		{
			$this->db->select("count(urm.project_id) as counts");
		}
		else
		{
			$this->db->select("urm.* ");
		...
		$this->db->from("project as urm");
		$this->db->order_by("project_id desc");
		
		if(!empty($params['project_id']))	
		{
			$this->db->where("urm.project_id" ,  $params['project_id']);
		}
		...
		if(!empty($params['record_status']))
		{
			...
			{
				$this->db->where("urm.project_id" ,  $params['record_status']);
			}
		}
