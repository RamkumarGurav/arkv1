...
class Expense_Model extends CI_Model
{
	...
	function get_expense($params = array())
	{
		$result='';
		if(!empty($params['search_for']))
		{
			$this->db->select("count(urm.expense_m_id) as counts");
		}
		else
		{
			$this->db->select("urm.* ");
		...
		$this->db->from("expense_master as urm");
		$this->db->order_by("expense_m_id desc");
		
		if(!empty($params['expense_m_id']))	
		{
			$this->db->where("urm.expense_m_id" ,  $params['expense_m_id']);
		}
		...
		if(!empty($params['record_status']))
		{
			...
			{
				$this->db->where("urm.expense_m_id" ,  $params['record_status']);
			}
		}
