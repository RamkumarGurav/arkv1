...
class Customer_Category_Model extends CI_Model
{
	...
	function get_customer_category_master($params = array())
	{
		$result='';
		if(!empty($params['search_for']))
		{
			$this->db->select("count(urm.customer_category_id) as counts");
		}
		else
		{
			$this->db->select("urm.* ");
		...
		$this->db->from("customer_category_master as urm");
		$this->db->order_by("customer_category_id desc");
		
		if(!empty($params['customer_category_id']))	
		{
			$this->db->where("urm.customer_category_id" ,  $params['customer_category_id']);
		}
		...
		if(!empty($params['record_status']))
		{
			...
			{
				$this->db->where("urm.customer_category_id" ,  $params['record_status']);
			}
		}
