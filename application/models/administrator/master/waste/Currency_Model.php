...
class Currency_Model extends CI_Model
{
	...
	function get_currency($params = array())
	{
		$result='';
		if(!empty($params['search_for']))
		{
			$this->db->select("count(urm.currency_id) as counts");
		}
		else
		{
			$this->db->select("urm.* ");
		...
		$this->db->from("currency as urm");
		$this->db->order_by("currency_id desc");
		
		if(!empty($params['currency_id']))	
		{
			$this->db->where("urm.currency_id" ,  $params['currency_id']);
		}
		...
		if(!empty($params['record_status']))
		{
			...
			{
				$this->db->where("urm.currency_id" ,  $params['record_status']);
			}
		}
