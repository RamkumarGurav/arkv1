<?php
class T_And_C_Quotation_Model extends CI_Model
{
	public $session_uid = '';
	public $session_name = '';
	...
	function get_t_and_c_quotation_master($params = array())
	{
		$result='';
		if(!empty($params['search_for']))
		{
			$this->db->select("count(urm.t_and_c_quotation_id) as counts");
		}
		else
		{
			$this->db->select("urm.* ");
		...
		$this->db->from("t_and_c_quotation_master as urm");
		$this->db->order_by("t_and_c_quotation_id desc");
		
		if(!empty($params['t_and_c_quotation_id']))	
		{
			$this->db->where("urm.t_and_c_quotation_id" ,  $params['t_and_c_quotation_id']);
		}
		...
		if(!empty($params['record_status']))
		{
			...
			{
				$this->db->where("urm.t_and_c_quotation_id" ,  $params['record_status']);
			}
		}
