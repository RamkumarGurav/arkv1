<?php
class Data_Source_Model extends CI_Model
{
	public $session_uid = '';
	public $session_name = '';
	...
	function get_data_source_master($params = array())
	{
		$result='';
		if(!empty($params['search_for']))
		{
			$this->db->select("count(urm.data_source_id) as counts");
		}
		else
		{
			$this->db->select("urm.* ");
		...
		$this->db->from("data_source_master as urm");
		$this->db->order_by("data_source_id desc");
		
		if(!empty($params['data_source_id']))	
		{
			$this->db->where("urm.data_source_id" ,  $params['data_source_id']);
		}
		...
		if(!empty($params['record_status']))
		{
			...
			{
				$this->db->where("urm.data_source_id" ,  $params['record_status']);
			}
		}
