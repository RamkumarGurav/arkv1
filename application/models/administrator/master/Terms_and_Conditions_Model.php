...
class Terms_and_Conditions_Model extends CI_Model
{
	...
	function get_terms_and_conditions_master($params = array())
	{
		$result='';
		if(!empty($params['search_for']))
		{
			$this->db->select("count(urm.terms_and_conditions_id) as counts");
		}
		else
		{
			$this->db->select("urm.* ");
		...
		$this->db->from("terms_and_conditions_master as urm");
		$this->db->order_by("terms_and_conditions_id desc");
		
		if(!empty($params['terms_and_conditions_id']))	
		{
			$this->db->where("urm.terms_and_conditions_id" ,  $params['terms_and_conditions_id']);
		}
		...
		if(!empty($params['record_status']))
		{
			...
			{
				$this->db->where("urm.terms_and_conditions_id" ,  $params['record_status']);
			}
		}
