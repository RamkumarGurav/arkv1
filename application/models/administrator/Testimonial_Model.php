...
class Testimonial_Model extends CI_Model
{
	...
	function get_testimonial($params = array())
	{
		$result='';
		if(!empty($params['search_for']))
		{
			$this->db->select("count(urm.testimonial_id) as counts");
		}
		else
		{
			$this->db->select("urm.* ");
		...
		$this->db->from("testimonial as urm");
		$this->db->order_by("testimonial_id desc");

		if(!empty($params['testimonial_id']))
		{
			$this->db->where("urm.testimonial_id" ,  $params['testimonial_id']);
		}
		...
		if(!empty($params['record_status']))
		{
			...
			{
				$this->db->where("urm.testimonial_id" ,  $params['record_status']);
			}
		}
