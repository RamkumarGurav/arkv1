...

{
...

	{
...

		{
			...
			$this->db->select("aau.product_id , aau.brand_id , aau.name, aau.status , aau.ref_code , aau.hsn_code, aau.tax_id, aau.short_description, aau.description, aau.application, aau.added_on, aau.is_bulk_enquiry, aau1.brand_name, pi.product_image_name, aau.updated_on");
	...
	function get_product_images($params = array())
	{
		...
		if(!empty($params['search_for']))
		{
			$this->db->select("distinct(count(aau.product_image_id)) as counts");
		}
		else
		{
			//$sql_get_list="select p.product_id , p.manufacturer_id , p.name, p.status , p.ref_code , p.slug_url , p.short_description, p.added_on, m.manufacturer_name from product p , manufacturer as m where m.manufacturer_id=p.manufacturer_id   ";
			...
			$this->db->group_by('aau.product_image_id');
		}
		$this->db->from("product_image as aau");
		$this->db->join("product_combination", "product_combination.product_image_id = aau.product_image_id and aau.product_id = product_combination.product_id", "left");
		if(!empty($params['product_id']))	
		{
			$this->db->where("aau.product_id" ,  $params['product_id']);
		}
...

	{
...

		{
			...
			$this->db->select("pc.* , ps.product_seo_id, ps.slug_url, ps.meta_title, ps.meta_description, ps.meta_keywords, pis.quantity, pi.product_image_id, pi.product_image_name");					
		...
		$this->db->join("product_in_store as pis" , "pis.product_combination_id = pc.product_combination_id" , "left");
